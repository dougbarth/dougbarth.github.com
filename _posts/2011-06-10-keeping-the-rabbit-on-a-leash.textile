...
With a correctly working token bucket, implementing fixed rate is fairly straightforward. First, when subscribing to a queue, we set an explicit prefetch_count on the channel and we set no_ack to false. The prefetch_count limits how many unacked messages RabbitMQ will deliver and no_ack allows us to acknowledge the message once we've finished processing it. In our application, we size the prefetch_count so there are a few seconds worth of messages waiting in the worker's memory to be sent.
